...
public class FriendsList extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            for (int i = 0; i < friends.size(); i++) {
                if (me.getUserID() == friends.get(i).getToID().getUserID() && friends.get(i).getStatus() == 2) {
                    userFriends.add(friends.get(i).getFromID().getUserName());
                } else if (me.getUserID() == friends.get(i).getFromID().getUserID() && friends.get(i).getStatus() == 2) {
                    userFriends.add(friends.get(i).getToID().getUserName());
                }
            }
        ...
        RequestDispatcher requestDispatcher = req.getRequestDispatcher("listOfFriends.jsp");
